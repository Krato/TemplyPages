<template>
    <div>
        <template v-for="number in field.field.repeatTimes" >
            <div class="p-4 m-4 text-90 border-b border-40 flex justify-between items-center bg-30" :key="'repeater_'+number">
            ...
            <template v-for="(childrenRepeater, keyRepeater, indexRepeater) in field.children">
                <template v-if="childrenRepeater.field.attribute == 'repeater'">
                    <div class="w-5/6 ml-auto" :rel="keyRepeater" :index="indexRepeater" :parentIndex="number" :key="'repeater_'+keyRepeater+'_'+number">
                        <repeater-field 
                ...
                <template v-else>
                    <div class="px-6" :key="'repeater_'+keyRepeater+'_'+number">
                        <component
                            ...
                            :field="getField(childrenRepeater.field, number, indexRepeater, childrenRepeater)"
                        >
                            <!-- :errors="errors" -->
                        </component>
                    </div>
